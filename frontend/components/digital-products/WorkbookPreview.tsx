...
export default function WorkbookPreview({ product }: { product: DigitalProduct }) {
  ...
  const clientWork = product.slug === "freelancer-agency-client-work-workbook";
  return (
    <div className="border border-[#b8cec5] bg-[#e8f1ee] p-4 shadow-[0_16px_35px_-22px_rgba(23,60,50,0.65)] sm:p-6">
      <div className="border border-[#b8cec5] bg-[#f7fbf8] p-4 sm:p-6">
        <div className="flex items-start justify-between gap-4 border-b border-[#c9dcd4] pb-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#28604e]">{product.previewLabel}</p><h2 className="mt-2 text-xl font-black text-[#173c32] sm:text-2xl">{clientWork ? "Client-work pipeline" : freelancer ? "Rate & quote planner" : "Monthly finance dashboard"}</h2></div><span className="border border-[#b8cec5] bg-white px-2 py-1 text-[10px] font-bold text-[#28604e]">SHEET 01</span></div>
        {clientWork ? <div className="mt-5 grid gap-3 sm:grid-cols-3"><Metric label="Quoted" value="₹40,250" /><Metric label="Outstanding" value="₹35,000" /><Metric label="Margin" value="42.3%" /></div> : freelancer ? <div className="mt-5 grid gap-3 sm:grid-cols-3"><Metric label="Monthly target" value="₹70,000" /><Metric label="Billable hours" value="64 hrs" /><Metric label="Hourly floor" value="₹1,093.75" /></div> : <div className="mt-5 grid gap-3 sm:grid-cols-3"><Metric label="Revenue" value="₹1,20,000" /><Metric label="Expenses" value="₹68,500" /><Metric label="Operating profit" value="₹51,500" /></div>}
        <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[420px] border-collapse text-left text-xs"><thead><tr className="bg-[#dcece5] text-[#28604e]"><th className="border border-[#b8cec5] px-3 py-2 font-bold">{clientWork ? "Stage" : "Month"}</th><th className="border border-[#b8cec5] px-3 py-2 font-bold">{clientWork ? "Project" : "Revenue"}</th><th className="border border-[#b8cec5] px-3 py-2 font-bold">{clientWork ? "Next review" : "Expenses"}</th><th className="border border-[#b8cec5] px-3 py-2 font-bold">{clientWork ? "Status" : "Margin"}</th></tr></thead><tbody>{(clientWork ? [["Quote", "DEMO - Website setup", "Scope", "Accepted"], ["Delivery", "DEMO - Brand refresh", "Review", "In progress"], ["Payment", "DEMO - Example Studio", "Follow-up", "Open"]] : [["April", "₹98,000", "₹61,200", "37.6%"], ["May", "₹1,08,000", "₹64,750", "40.0%"], ["June", "₹1,20,000", "₹68,500", "42.9%"]]).map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={`${row[0]}-${index}`} className="border border-[#c9dcd4] px-3 py-2 text-[#173c32]">{cell}</td>)}</tr>)}</tbody></table></div>
