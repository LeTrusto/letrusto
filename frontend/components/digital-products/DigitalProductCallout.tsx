...
export default function DigitalProductCallout({ freelancer = false, clientWork = false }: { freelancer?: boolean; clientWork?: boolean }) {
  ...
  return <aside className="mt-16 border-l-4 border-[var(--lt-accent)] bg-[var(--surface-soft)] p-5 sm:p-6"><p className="lt-eyebrow">Go deeper</p><h2 className="mt-2 text-lg font-black text-[var(--text-primary)]">{product.title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--text-secondary)]">{product.description}</p><Link href={`/digital-products/${product.slug}`} onClick={() => trackSafeEvent("digital_product_cta_clicked", { product_name: getDigitalProductBySlug(product.slug)?.name ?? product.slug, product_slug: product.slug, interaction: "view_product", source_tool: pathname.split("/").filter(Boolean)[1] ?? "" })} className="lt-btn lt-btn-sm lt-btn-secondary mt-4">View the toolkit <ArrowUpRight size={15} aria-hidden="true" /></Link></aside>;
}