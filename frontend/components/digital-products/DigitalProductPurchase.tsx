  ...
  async function startCheckout() {
    ...
    try {
      ...
      const checkout = new window.Razorpay({ key: order.key_id, amount: order.amount, currency: order.currency, order_id: order.razorpay_order_id, name: "LeTrusto", description: product.name, prefill: { name: user?.full_name ?? "", email: user?.email ?? "", contact: "" }, handler: async (result: RazorpayResult) => {
        try { await verifyDigitalPayment(accessToken, product.slug, result); trackSafeEvent("digital_product_cta_clicked", { product_name: product.name, product_slug: product.slug, interaction: "payment_verified" }); setState("success"); setMessage("Payment verified. Your download is ready."); }
      ...
      checkout.on?.("payment.failed", (failure) => { setState("error"); setMessage(failure.error?.description ?? "Payment failed."); });
      checkout.open();
