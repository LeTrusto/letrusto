...
export default function DigitalProductPurchase({ product }: { product: DigitalProduct }) {
  ...
  return <aside id="purchase" className="lt-card border-[var(--lt-primary)]"><Script src="https://checkout.razorpay.com/v1/checkout.js" strategy="afterInteractive" /><p className="lt-eyebrow">Digital delivery</p><div className="mt-3 flex items-end justify-between gap-4"><p className="text-3xl font-black text-[var(--text-primary)]">{formatDigitalProductPrice(product)}</p><span className="text-right text-xs font-semibold text-[var(--text-muted)]">One-time<br />purchase</span></div>{!isAuthenticated && !isLoading ? <div className="mt-6 border border-[var(--border)] bg-[var(--surface-soft)] p-4"><p className="flex items-center gap-2 text-sm font-bold text-[var(--text-primary)]"><LockKeyhole size={17} aria-hidden="true" /> Sign in to purchase</p><p className="mt-2 text-sm leading-6 text-[var(--text-secondary)]">Your download is attached to your LeTrusto account.</p><Link href={`/login?next=/digital-products/${product.slug}#purchase`} onClick={() => trackSafeEvent("digital_product_auth_required", { product_name: product.name, product_slug: product.slug })} className="lt-button-primary mt-4 inline-flex" aria-label={`Buy ${product.name} now`}>BUY NOW</Link></div> : state === "success" ? <div className="mt-6 border border-emerald-700 bg-emerald-50 p-4" role="status"><p className="flex items-center gap-2 text-sm font-bold text-emerald-900"><CheckCircle2 size={17} aria-hidden="true" /> {message}</p><button type="button" onClick={download} className="lt-button-primary mt-4 inline-flex items-center gap-2"><Download size={16} aria-hidden="true" /> Download toolkit</button></div> : <div className="mt-6"><button type="button" onClick={startCheckout} disabled={state === "working" || isLoading} aria-label={`Buy ${product.name} now`} className="lt-button-primary inline-flex w-full items-center justify-center gap-2">{state === "working" && <Loader2 size={16} className="animate-spin" aria-hidden="true" />}{state === "working" ? "Preparing secure checkout" : "BUY NOW"}</button>{message && <p className="mt-3 text-sm text-red-700" role="alert">{message}</p>}</div>}<p className="mt-4 text-xs leading-5 text-[var(--text-muted)]">Protected digital delivery. This product never enters the physical cart and does not use shipping or Printful fulfillment.</p></aside>;
}