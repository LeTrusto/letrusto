    ...
    <article className="lt-card lt-card-hover flex h-full flex-col overflow-hidden p-0">
      <div className="border-b border-[var(--border)] bg-[#e8f1ee] p-5">
        <div className="flex aspect-[16/10] flex-col justify-between border border-[#b8cec5] bg-[#f7fbf8] p-4 shadow-sm">
          ...
          <div><p className="text-xs font-semibold text-[#28604e]">{product.previewLabel}</p><p className="mt-1 text-xl font-black text-[#173c32]">{product.slug === "freelancer-rate-project-pricing-toolkit" ? "Rate &amp; quote planning" : "Finance &amp; pricing"}</p></div>
          <div className="grid grid-cols-3 gap-2 text-[10px] font-semibold text-[#28604e]"><span className="border border-[#b8cec5] bg-white p-2">{product.slug === "freelancer-rate-project-pricing-toolkit" ? "Rates" : "Pricing"}</span><span className="border border-[#b8cec5] bg-white p-2">{product.slug === "freelancer-rate-project-pricing-toolkit" ? "Projects" : "Expenses"}</span><span className="border border-[#b8cec5] bg-white p-2">{product.slug === "freelancer-rate-project-pricing-toolkit" ? "Review" : "Break-even"}</span></div>
        </div>
      </div>
