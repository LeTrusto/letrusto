...
export function PropertyGallery({ title, media }: { title: string; media: PublicPropertyMedia[] }) {
  ...
  return <section className="detail-gallery" aria-label={`${title} media gallery`}>
    <div className="detail-gallery-main">
      {selected.media_type === "VIDEO" ? <video src={selected.public_url ?? undefined} controls playsInline aria-label={selected.caption ?? `${title} video`} /> : <Image src={selected.public_url!} alt={selected.caption ?? `${title} media ${selectedIndex + 1}`} fill sizes="(max-width: 768px) 100vw, 68vw" priority={selectedIndex === 0} unoptimized />}
    ...
    {usableMedia.length > 1 && <div className="detail-media-rail" role="tablist" aria-label={`${title} media choices`}>
      {usableMedia.map((item, index) => <button type="button" role="tab" aria-selected={selectedIndex === index} className={`detail-media-thumb ${selectedIndex === index ? "is-selected" : ""}`} key={item.id} onClick={() => setSelectedIndex(index)}>
        ...
        {item.media_type === "VIDEO" ? <span className="video-thumb">Video</span> : <Image src={item.public_url!} alt={item.caption ?? `${title} media ${index + 1}`} fill sizes="150px" unoptimized />}
      </button>)}
    </div>}
  </section>;
}
