      ...
      <div className="grid auto-rows-fr gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            className="group relative isolate h-full overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white p-3.5 shadow-sm transition duration-500 ease-out hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-[0_20px_56px_-30px_rgba(15,23,42,0.34)]"
          ...
          >
            <div className="relative flex h-full min-h-[292px] flex-col">
              ...
              <div className="mt-3.5 flex-1">
                ...
                <p className="mt-1 min-h-[2.75rem] line-clamp-2 text-xs leading-relaxed text-slate-600">{item.description}</p>
              </div>

              <span className="mt-auto inline-flex items-center gap-1.5 pt-3 text-xs font-semibold text-slate-900 transition duration-300 group-hover:translate-x-0.5 group-hover:text-violet-700">
