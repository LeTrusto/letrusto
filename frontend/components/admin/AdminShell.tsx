...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="admin-app"><header className="admin-topbar"><Link href="/admin" className="admin-brand"><span className="mark-dot" />BENGALURU PROPERTY <small>OPERATIONS</small></Link><div className="admin-user"><span>{user?.full_name || user?.email}</span><NotificationCenter admin /><button type="button" onClick={() => void logout()} aria-label="Sign out"><LogOut size={16} /></button></div></header><div className="admin-layout"><aside className="admin-sidebar"><p className="admin-eyebrow">Control room</p><nav aria-label="Admin navigation">{nav.map(({ href, label, icon: Icon }) => <Link key={href} className={pathname === href || (href !== "/admin" && pathname.startsWith(href)) ? "active" : ""} href={href}><Icon size={16} />{label}</Link>)}</nav></aside><main className="admin-main">{children}</main></div></div>;
}
