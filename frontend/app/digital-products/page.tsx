...
export default function DigitalProductsPage() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-16 md:px-6 md:py-24">
      ...
      <section className="mt-14 border-y border-[var(--border)] py-8"><div className="grid gap-6 md:grid-cols-5">{DIGITAL_PRODUCT_CATEGORIES.map((category) => <div key={category.slug} className="group"><p className="text-sm font-black text-[var(--text-primary)]">{category.name}</p><p className="mt-2 text-xs leading-5 text-[var(--text-secondary)]">{category.description}</p></div>)}</div></section>
      <section id="published" className="mt-14"><div className="flex items-end justify-between gap-5"><div><p className="lt-eyebrow">Published now</p><h2 className="lt-heading-2 mt-2">Built for practical decisions</h2></div><p className="text-sm text-[var(--text-muted)]">{products.length} products published</p></div><div className="mt-6 grid gap-6 lg:grid-cols-2">{products.map((product) => <DigitalProductCard key={product.id} product={product} />)}</div></section>
