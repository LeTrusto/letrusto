...
export default async function DigitalProductPage({ params }: ProductPageProps) {
  ...
  return (
    <main className="bg-[var(--background)]">
      ...
      <section className="bg-[#26113c] text-white"><div className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12"><Link href="/digital-products" className="text-sm font-semibold text-purple-200 hover:text-white">&larr; All digital products</Link><div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.78fr)] lg:items-center"><div><p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-pink-300"><FileSpreadsheet size={15} /> {product.category.name} / {product.format.split(",")[0]}</p><h1 className="mt-5 max-w-3xl text-4xl font-black leading-tight tracking-[-0.03em] md:text-6xl">{product.name}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-purple-100">{product.valueProposition}</p><div className="mt-7 flex flex-wrap gap-x-6 gap-y-3 text-sm text-purple-200"><span className="inline-flex items-center gap-2"><Check size={16} className="text-pink-300" />Editable workbook</span><span className="inline-flex items-center gap-2"><Download size={16} className="text-pink-300" />Protected digital delivery</span></div></div><div className="rounded-2xl border border-white/15 bg-white/10 p-3 shadow-2xl backdrop-blur-sm"><WorkbookPreview product={product} /></div></div></div></section>
      <div className="mx-auto max-w-7xl px-4 py-10 md:px-6 md:py-16"><div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.72fr)] lg:items-start"><div><div className="grid gap-3 sm:grid-cols-3"><Info icon={FileSpreadsheet} label="Format" value="Editable CSV" /><Info icon={Download} label="Delivery" value="Protected download" /><Info icon={LockKeyhole} label="Access" value="Your account" /></div><section className="mt-12"><p className="lt-eyebrow">Built for the problem in front of you</p><h2 className="lt-heading-2 mt-3">A clearer way to {product.slug.includes("finance") ? "run the numbers" : product.slug.includes("rate-project") ? "price your work" : "run client work"}.</h2><p className="lt-body mt-4 max-w-2xl">{product.description}</p></section><section className="mt-12 border-t border-[var(--border)] pt-10"><h2 className="lt-heading-2">What you receive</h2><ul className="mt-6 grid gap-3 sm:grid-cols-2">{product.included.map((item) => <li key={item} className="flex gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-4 text-sm leading-6 text-[var(--text-secondary)]"><Check size={17} className="mt-0.5 shrink-0 text-[var(--lt-accent)]" />{item}</li>)}</ul></section><section className="mt-12 border-t border-[var(--border)] pt-10"><h2 className="lt-heading-2">Who it is for</h2><div className="mt-5 flex flex-wrap gap-2">{product.audience.map((item) => <span key={item} className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-sm font-semibold text-[var(--text-secondary)]">{item}</span>)}</div></section></div><div className="lg:sticky lg:top-28"><DigitalProductPurchase product={product} /></div></div>
