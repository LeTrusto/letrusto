...
import { createWidget, deactivateWidget, getWidgetEvents, getWidgets, updateWidget, type Widget, type WidgetDraft } from "@/services/saas.service";
import { recordMarketingEvent } from "@/services/marketing.service";

const emptyDraft: WidgetDraft = { name: "My first widget", domain_name: "", theme_color: "#f97316", position: "bottom-left", display_delay: 3, is_active: true };
...
export default function WidgetsPage() {
  ...
  const embedCode = selected ? `<script src="https://letrusto.com/widget.js" data-api-base="https://letrusto.com" data-id="${selected.id}" async></script>` : "";
  ...
  useEffect(() => {
    ...
    getWidgets(accessToken).then((items) => {
      ...
      if (items[0]) {
        ...
        getWidgetEvents(accessToken, items[0].id).then((events) => setHasEvent(events.length > 0)).catch(() => undefined);
      }
    }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Could not load widgets.")).finally(() => setLoading(false));
  }, [accessToken]);

  function selectWidget(widget: Widget) { setSelectedId(widget.id); setDraft(toDraft(widget)); setHasEvent(false); setEmbedCodeCopied(false); setError(""); getWidgetEvents(accessToken ?? "", widget.id).then((events) => setHasEvent(events.length > 0)).catch(() => undefined); }
  function startNew() { setSelectedId(null); setDraft(emptyDraft); setHasEvent(false); setEmbedCodeCopied(false); setInstallationMessage(""); setError(""); }
  ...
  async function save() {
    ...
    try {
      ...
      if (!selectedId) {
        trackSafeEvent("widget_created", { source: "widget_dashboard" });
        void recordMarketingEvent("widget_created", { source: "widget_dashboard" }).catch(() => undefined);
      }
  ...
  async function copyCode() {
    ...
    trackSafeEvent("embed_code_viewed", { source: "widget_dashboard" });
    void recordMarketingEvent("embed_code_viewed", { source: "widget_dashboard" }).catch(() => undefined);
    window.setTimeout(() => setCopied(false), 1600);
  }

  function openInstallationCheck() {
    if (!selected?.domain_name) return;
    const domain = /^https?:\/\//i.test(selected.domain_name) ? selected.domain_name : `https://${selected.domain_name}`;
    window.open(domain, "_blank", "noopener,noreferrer");
    setInstallationMessage("Your site opened in a new tab. Confirm the proof popup appears there, then return here.");
  }
  ...
  return (
    <div>
      ...
      <OnboardingChecklist hasWidget={widgets.length > 0} hasEvent={hasEvent} hasEmbedCode={embedCodeCopied} />
...
function OnboardingChecklist({ hasWidget, hasEvent, hasEmbedCode }: { hasWidget: boolean; hasEvent: boolean; hasEmbedCode: boolean }) {
  const steps = [
    { label: "Create your first widget", complete: hasWidget },
    { label: "Add a real customer event", complete: hasEvent },
    { label: "Copy the embed code to your site", complete: hasEmbedCode },
  ];
  return <section className="mt-7 border border-[#b9cec2] bg-[#edf5f1] p-4 sm:p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#0f766e]">Activation path</p><h3 className="mt-1 text-lg font-black">Get your first proof widget live</h3></div><span className="text-xs font-bold text-[#587268]">{steps.filter((step) => step.complete).length} / {steps.length} complete</span></div><ol className="mt-4 grid gap-2 sm:grid-cols-3">{steps.map((step, index) => <li key={step.label} className={`flex items-center gap-2 border bg-white px-3 py-3 text-xs font-bold ${step.complete ? "border-[#0f766e] text-[#0f766e]" : "border-[#d9e5df] text-[#587268]"}`}><span className={`flex h-5 w-5 shrink-0 items-center justify-center text-[10px] ${step.complete ? "bg-[#0f766e] text-white" : "bg-[#e8f0ec]"}`}>{step.complete ? <Check className="h-3 w-3" /> : index + 1}</span>{step.label}</li>)}</ol></section>;
}
