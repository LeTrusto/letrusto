...
function Field({ id, label, value, onChange, placeholder, suffix, help }: { id: string; label: string; value: string; onChange: (value: string) => void; placeholder: string; suffix?: string; help: string }) { return <div><label htmlFor={id} className="lt-label mb-2 block">{label}</label><div className="flex h-[46px] overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface)] focus-within:border-[var(--ring)] focus-within:ring-2 focus-within:ring-[var(--ring)]/20">{suffix ? <input id={id} className="min-w-0 flex-1 border-0 bg-transparent px-3 py-2 text-[var(--text-primary)] outline-none" type="text" inputMode="decimal" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} aria-describedby={`${id}-help`} /> : <><span className="flex w-11 shrink-0 items-center justify-center border-r border-[var(--border)] text-sm font-semibold text-[var(--text-muted)]" aria-hidden="true">₹</span><input id={id} className="min-w-0 flex-1 border-0 bg-transparent px-3 py-2 text-[var(--text-primary)] outline-none" type="text" inputMode="decimal" value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} aria-describedby={`${id}-help`} /></>} {suffix && <span className="flex w-11 shrink-0 items-center justify-center border-l border-[var(--border)] text-sm font-semibold text-[var(--text-muted)]" aria-hidden="true">{suffix}</span>}</div><p id={`${id}-help`} className="mt-1.5 text-xs text-[var(--text-muted)]">{help}</p></div>; }
