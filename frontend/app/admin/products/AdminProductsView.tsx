...
function PrintfulProductWorkflow({ product, working, onSave }: { product: Product; working: boolean; onSave: (payload: { india_price_inr: string; international_price_usd: string; shipping_reviewed: boolean }) => void }) {
  ...
  const [shippingReviewed] = useState(savedPricing.shipping_reviewed);
  const [shippingRates, setShippingRates] = useState<PrintfulShippingRate[]>([]);
  const activeVariants = product.variants.filter((variant) => variant.active);
  useEffect(() => {
    void fetch(`${API_BASE}/api/v1/admin/products/${product.id}/printful-shipping`, { headers: apiHeaders() })
      .then((response) => response.ok ? response.json() as Promise<PrintfulShippingRate[]> : Promise.reject(new Error("Unable to load shipping rates")))
