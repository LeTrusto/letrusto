...
class Order(Base):
    ...
    shipped_email_sent_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    delivered_email_sent_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    notification_failure_reason: Mapped[str | None] = mapped_column(String(500), nullable=True)
...
class WidgetUsage(Base):
    ...
    __table_args__ = (UniqueConstraint("widget_id", "period_start", name="uq_widget_usage_period"),)
